...
def main(begin:str, end:str):
	...
	path_dlx = 'Data/dlx'
	path_sos = 'Data/sos'
	path_smoothed_r1h = 'Data/afsr1h_avgmmd3_smooth'

	da_r1h = create_xarray(path_r1h + '/*.tif')
	print('downloaded r1h')
	...
	das = [da_r1h, da_vim, da_tda, da_dlx]
	...
	# Weights computed from smoothed r1h data
	da_r1h_smooth = create_xarray(path_smoothed_r1h + '/*.rst', avg = True)

	# Selecting months of interest
	da_r1h_smooth_sel = da_r1h_smooth.sel(time = get_months(begin, end))

	# Creating mask
	ws = da_r1h_smooth_sel.sum('time')
	w_mask = ws.where(ws == 0, 1)

	# Computing weights
	weights = da_r1h_smooth_sel / ws
	...
	Qmulti = Qr1h.copy()
	Qmulti.values = coef_r1h * Qr1h.values + coef_vim * Qvim.values + coef_tda * Qtda.values + coef_dlx * Qdlx.values + coef_sos * Qsos.values
